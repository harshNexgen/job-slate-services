  ...
  try {
    if (event.queryStringParameters != null) {
      if (
        ...
        event.queryStringParameters.offset == null
      ) {
  ...
  try {
    ...
    if (!companyId) {
      return createResponse(400, {
        response: "Company Id Not found in database",
      });
    }
...
export const getAllCategory = async (event, context) => {
  try {
    ...
    if (
      !getCategories ||
      getCategories[0] == null ||
      getCategories[0] == undefined
    ) {
    ...
    if (event.queryStringParameters != null) {
      ...
      const data = await Postings.find({
        $and: [
          {
            $or: [
              { "salary.maxAmount": { $lt: max } },
              { "salary.maxAmount": { $eq: max } },
            ],
          },
        ],
      });
      ...
      return createResponse(400, {
        response: "Required query string not found",
      });
    }
...
export const getJobByLocation = async (event, context) => {
  ...
  const data = await Postings.find({
    $or: [{ city: city }, { location: location }],
  });
  console.log(data[0]);
  if (data[0] != undefined) {
    return createResponse(200, { response: data });
...
export const getJobFilter = async (event, context) => {
  let page;
  let limit;
  ...
  const queryfindExpressions = { ...event.queryStringParameters };
  delete queryfindExpressions.page;
  delete queryfindExpressions.limit;
  console.log(queryfindExpressions);
   const query = await Postings.find({
    ...queryfindExpressions,
  }).skip(startIndex).limit(limit); 
  console.log(query.length,"the length of the data");
